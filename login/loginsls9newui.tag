...
var date_offset = 2; // (current day + 5 working days) HBL change to 3
...
for(var i = 0; i < pending.length; i++){
	...
	var level = row[3].trim().toLowerCase();
	if(containsSubstring(level, "sec")){
		level = "sec";
	}
	else if(containsSubstring(level, "jc") || containsSubstring(level, "college")){
		level = "jc";
	}
...
// WRITING TO FILE
write `csv_row(["S/N", "Title", "Subject", "Level", "School", "Date Submitted", "Author", "URL", "Similar", "Resubmitted", "Assigned To", "Date Lesson Assigned", "Due Date", "Duplicate", "Similar To", "Email of Admin"])` to pendingreview_list.csv
