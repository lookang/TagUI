...
for i from 1 to total
	...
	if (similar == 0)
		...
		js link = url()
		// read the title
		// read output-text to title // remove this to avoid 2nd tab read incorrectly for chinese/tamil words 
		echo `title`
		echo `link`
		dom window.close()
		}
	...
	echo `pending`
	js rowCnt++;
	js mutlipleof20 = total%20
	echo multipleof20 = `mutlipleof20`
	// goal is to skip this rowCnt == 21 if total is 20,40,60 etc
	if ((rowCnt == 21) and (mutlipleof20 != 0))
		{
		click //button[@aria-label="Next page"]
